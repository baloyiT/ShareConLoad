...
export async function approveTransporter(transporterProfileId: string): Promise<{ error?: string }> {
  ...
  if (!ap) return { error: 'Transporter profile not found.' };
  ...
  if (profile?.user_id) {
    await supabase.from('notifications').insert({
      ...
      message: 'Congratulations! Your transporter application has been approved. You can now access the Transporter Portal.',
      type:    'success',
    });
  }
...
export async function rejectTransporter(transporterProfileId: string, reason: string): Promise<{ error?: string }> {
  ...
  if (profile?.user_id) {
    await supabase.from('notifications').insert({
      ...
      message: `Your transporter application was not approved. Reason: ${reason}. You may resubmit after addressing the issues.`,
      type:    'warning',
    });
  }
