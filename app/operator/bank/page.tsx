...
export default function OperatorBankPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!profile) { setSubmitError('Operator profile not found.'); setLoadingProfile(false); return; }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isManual && !manualBankName.trim())     { setSubmitError('Bank name is required.'); return; }
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-5">
            ...
            <div>
              ...
              <select value={bankCountry ?? ''} onChange={(e) => { setBankCountry(e.target.value); setBankCode(''); setSwiftCode(''); setManualBankName(''); }}
            ...
            {/* ── Notice (shown once country is selected) ── */}
            {bankCountry && (
              isManual ? (
                <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800">
                  ...
                  <span>
                    ...
                    Save your details and our team will contact you to arrange international transfers.
                  </span>
                </div>
              ) : (
                <div className="flex items-start gap-2.5 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-xs text-blue-700">
                  ...
                  <span>
                    Automatic bank verification is not available for {bankCountry}.
              ...
              <>
                ...
                <div>
                  ...
                  <input type="text" inputMode="numeric" value={bankAccountNumber ?? ''}
                    ...
                    className="input input-bordered w-full text-sm font-mono" maxLength={20} />
                </div>
              </>
            )}

            {/* ── Manual: bank name + SWIFT + account number ── */}
            {isManual && bankCountry && (
              <>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                ...
                <div>
                  ...
                  <input type="text" value={swiftCode ?? ''}
                    ...
                    className="input input-bordered w-full text-sm font-mono" maxLength={11} />
                </div>

                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                    Account / IBAN Number <span className="text-gray-400 font-normal">(optional)</span>
                  </label>
            ...
            {/* ── Account Holder Name (always last among bank fields) ── */}
            {bankCountry && (
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  Account Holder Name <span className="text-red-500">*</span>
                </label>
                <p className="text-xs text-gray-400 mb-1.5">Full name or business name as it appears on your bank account.</p>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
              Payouts will be sent to this account and cannot be reversed once initiated.
              Double-check all details before registering.
            </div>
