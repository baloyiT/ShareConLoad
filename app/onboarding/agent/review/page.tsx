  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'linear-gradient(135deg, #0b103a 0%, #1a3a6b 100%)' }}>
      <nav className="flex items-center px-6 py-4">
        <Link href="/onboarding/agent/bank" className="flex items-center gap-2.5">
          ...
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === 4 ? 'bg-green-500 text-white' : 'bg-white/60 text-gray-700'}`}>
                {i < 4 ? <Check className="w-3.5 h-3.5" strokeWidth={3} /> : '5'}
              </div>
              ...
              <div className="pt-1 flex flex-col gap-1">
                {[
                  ...
                  <div key={label} className="flex items-center justify-between">
                    ...
                    {url ? <span className="text-green-600 font-medium flex items-center gap-1"><Check className="w-3.5 h-3.5" strokeWidth={3} /> Uploaded</span> : <span className="text-red-500">Missing</span>}
                  </div>
                ))}
              </div>
            </div>
